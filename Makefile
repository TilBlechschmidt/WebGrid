...
run:
	...
	sleep 1
	docker run -it -d --rm --network webgrid --name webgrid-metrics -e RUST_LOG=debug,warp=warn -p 3036:3036 webgrid-metrics
	docker run -it -d --rm --network webgrid --name webgrid-proxy-1 -e RUST_LOG=debug,hyper=warn -p 80:8080 webgrid-proxy
